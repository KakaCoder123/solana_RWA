...
- [ ] **1.0.1** Установить Rust: `rustup install 1.81.0` + `rustup default 1.81.0`
- [ ] **1.0.2** Установить Solana CLI 2.1.x: `sh -c "$(curl -sSfL https://release.solana.com/v2.1.0/install)"`
- [ ] **1.0.3** Установить Anchor 0.32.x: `cargo install --git https://github.com/coral-xyz/anchor avm && avm install 0.32.0 && avm use 0.32.0`
- [ ] **1.0.4** Создать Devnet wallet: `solana-keygen new` + `solana config set --url devnet`
- [ ] **1.0.5** Получить Devnet SOL: `solana airdrop 2`
- [ ] **1.0.6** Создать Anchor проект: `anchor init vendchain-contracts`
...
#### Программа 2 — `vend_staking`:
- [ ] **1.2.1** PDA: `StakingPool { total_staked, reward_rate, last_update }`
- [ ] **1.2.2** PDA: `UserStake { owner, amount, staked_at, rewards_pending }`
- [ ] **1.2.3** Инструкция `stake(amount)` — перевод VEND в пул
- [ ] **1.2.4** Инструкция `unstake(amount)` — с lockup периодом (7 дней)
- [ ] **1.2.5** Инструкция `claim_rewards` — накопленные награды
- [ ] **1.2.6** Инструкция `distribute_rewards(amount)` — вызывается из treasury
- [ ] **1.2.7** Написать тесты (stake → wait → claim → unstake)
- [ ] **1.2.8** Задеплоить на Devnet
