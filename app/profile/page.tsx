...
const ACTIVITY = [
  { type: 'Earn',  icon: '💰', bg: 'rgba(20,241,149,0.1)',  amt: '+125.00 VEND',   usd: '$58.20',    date: 'Oct 24, 2023', hash: '7Gz2...9xpL' },
  { type: 'Stake', icon: '🔒', bg: 'rgba(153,69,255,0.1)', amt: '-5,000.00 VEND', usd: '$2,328.00', date: 'Oct 22, 2023', hash: '9fX1...4kMo' },
  { type: 'Buy',   icon: '🔄', bg: 'rgba(0,194,255,0.1)',  amt: '+850.20 VEND',   usd: '$396.19',   date: 'Oct 19, 2023', hash: '2wQ9...3vRt' },
]
...
// ── Countdown ──────────────────────────────────────────────────
function Countdown() {
  const [t, setT] = useState({ d: 14, h: 8, m: 22, s: 45 })
  useEffect(() => {
    const iv = setInterval(() => setT(p => {
      ...
      if (h < 0) { h = 23; d-- }
      return {
        d: Math.max(0, d), h: Math.max(0, h),
        m: Math.max(0, m), s: Math.max(0, s),
      }
    }), 1000)
...
export default function ProfilePage() {
  ...
  // Only redirect after mount + wallet confirmed not connected (not just loading)
  useEffect(() => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: '#14F195', background: 'rgba(20,241,149,0.08)', padding: '3px 7px', borderRadius: 5 }}>↑ +12.4%</div>
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#fff', marginBottom: 4, letterSpacing: '-0.5px' }}>24,500.50</div>
              <div style={{ fontSize: 11, color: '#64748b' }}>≈ $592.10 USD</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: '#14F195', background: 'rgba(20,241,149,0.08)', padding: '3px 7px', borderRadius: 5 }}>↑ +3.1%</div>
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#fff', marginBottom: 4, letterSpacing: '-0.5px' }}>84.22</div>
              <div style={{ fontSize: 11, color: '#64748b' }}>≈ $11,450.12 USD</div>
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: '#475569' }}>3 positions</div>
                  </div>
                </div>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#818cf8', background: 'rgba(99,102,241,0.1)', padding: '3px 7px', borderRadius: 5 }}>18.5% APY</div>
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#fff', marginBottom: 4, letterSpacing: '-0.5px' }}>15,000</div>
              <div style={{ fontSize: 11, color: '#64748b' }}>VEND locked</div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: '#475569' }}>
                  Accumulated {period === '30D' ? '30' : period === '90D' ? '90' : '365'}-day ecosystem earnings
                </p>
              </div>
          ...
          <div style={{ ...card, padding: 26 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <h2 style={{ fontSize: 15, fontWeight: 900, letterSpacing: 1 }}>RECENT ACTIVITY</h2>
              <span style={{ fontSize: 13, color: '#6366f1', fontWeight: 600, cursor: 'pointer' }}>View All →</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 110px 100px 90px', gap: 8, padding: '0 12px 10px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
              {ACTIVITY.map((tx, i) => (
                <div key={i} style={{
                  display: 'grid', gridTemplateColumns: '90px 1fr 110px 100px 90px', gap: 8,
                  padding: '12px', borderRadius: 10,
                  background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)',
                  alignItems: 'center',
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: tx.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>{tx.icon}</div>
                    <span style={{ fontSize: 13, color: '#94a3b8' }}>{tx.type}</span>
                  </div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700 }}>{tx.amt}</div>
                    <div style={{ fontSize: 11, color: '#475569' }}>{tx.usd}</div>
                  </div>
                  <div style={{ fontSize: 12, color: '#475569' }}>{tx.date}</div>
                  <div style={{ fontSize: 12, color: '#475569', fontFamily: 'monospace' }}>{tx.hash}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 32, fontWeight: 900, lineHeight: 1 }}>12</div>
                <div style={{ fontSize: 10, color: '#475569', letterSpacing: 1, marginTop: 4 }}>INVITES</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 20, fontWeight: 900, color: '#14F195' }}>+420 VEND</div>
          ...
          <div style={{ ...card, padding: 20 }}>
            ...
            {/* VAULT OMEGA */}
            <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 16, marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                <div style={{ fontSize: 9, color: '#14F195', fontWeight: 700, letterSpacing: 1.2 }}>VAULT OMEGA</div>
                <div style={{ fontSize: 9, color: '#475569', letterSpacing: 1 }}>APY</div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14 }}>
                <div style={{ fontSize: 18, fontWeight: 900 }}>10,000 VEND</div>
                <div style={{ fontSize: 20, fontWeight: 900, color: '#14F195' }}>18.5%</div>
              </div>
              <div style={{ fontSize: 9, color: '#475569', letterSpacing: 1.2, marginBottom: 10 }}>UNLOCKS IN</div>
              <Countdown />
              <button style={{
                width: '100%', padding: '9px', borderRadius: 8, cursor: 'pointer',
                background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)',
                color: '#94a3b8', fontSize: 11, fontWeight: 700, letterSpacing: 1, transition: 'background 0.2s',
              }}
            ...
            <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                <div style={{ fontSize: 9, color: '#9945FF', fontWeight: 700, letterSpacing: 1.2 }}>VAULT ALPHA</div>
                <div style={{ fontSize: 9, color: '#475569', letterSpacing: 1 }}>APY</div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14 }}>
                <div style={{ fontSize: 18, fontWeight: 900 }}>5,000 VEND</div>
                <div style={{ fontSize: 20, fontWeight: 900, color: '#14F195' }}>12.2%</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, fontSize: 12, color: '#14F195', fontWeight: 600 }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#14F195', display: 'inline-block' }} className="pulse-dot" />
                Ready to withdraw
              </div>
              <button style={{
                width: '100%', padding: '9px', borderRadius: 8, cursor: 'pointer',
                background: 'linear-gradient(135deg, #9945FF, #6366f1)',
                border: 'none', color: '#fff', fontSize: 11, fontWeight: 700, letterSpacing: 1, transition: 'opacity 0.2s',
              }}
                onMouseEnter={e => (e.currentTarget.style.opacity = '0.85')}
                onMouseLeave={e => (e.currentTarget.style.opacity = '1')}
              >WITHDRAW ASSETS</button>
            </div>
          </div>
